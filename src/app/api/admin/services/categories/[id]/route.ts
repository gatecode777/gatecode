...
import { getServiceCategoryById, updateServiceCategory, deleteServiceCategory } from '@/lib/services/servicesService';
...
export async function GET(req: NextRequest, { params }: Ctx) {
  ...
  const cat = await getServiceCategoryById(id);
  if (!cat) return NextResponse.json<ApiResponse>({ success:false, message:'Not found' }, { status:404 });
...
export async function PATCH(req: NextRequest, { params }: Ctx) {
  ...
  try {
    ...
    const { valid, errors } = validateServiceCategory(body, true);
    ...
    const updated = await updateServiceCategory(id, body);
...
export async function DELETE(req: NextRequest, { params }: Ctx) {
  if (!auth(req)) return NextResponse.json<ApiResponse>({ success:false, message:'Unauthorized' }, { status:401 });
  const { id } = await params;
  const ok = await deleteServiceCategory(id);
