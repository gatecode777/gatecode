...
import {
  HeroBlockEditor, IntroBlockEditor, CardsBlockEditor, TextWithImageBlockEditor,
  ProcessBlockEditor, IndustriesBlockEditor, TextBlockEditor, BulletsBlockEditor,
  ImageGridBlockEditor, CTABlockEditor, TwoColumnBlockEditor, DividerBlockEditor,
  ButtonsBlockEditor, XIcon, PlusIcon, GripIcon,
} from './blocks/ServiceBlockEditors';
...
// ── Block type registry ────────────────────────────────────────────────────
const BLOCK_TYPES: { type: ServiceBlockType; emoji: string; label: string; desc: string; defaultData: Record<string,unknown> }[] = [
  { type:'hero',          emoji:'🖼️',  label:'Hero Banner',        desc:'Banner + title + subtitle + CTA',       defaultData:{ title:'', subtitle:'', bannerImage:'', ctaLabel:'Get Free Consultation', ctaUrl:'' } },
  { type:'intro',         emoji:'📝',  label:'Introduction',        desc:'Rich text paragraph (centered)',         defaultData:{ body:'' } },
  { type:'cards',         emoji:'🃏',  label:'Service Cards',       desc:'Grid of title+description cards',        defaultData:{ title:'', cards:[] } },
  { type:'textWithImage', emoji:'📰',  label:'Text + Image',        desc:'Heading, body, bullets & side image',    defaultData:{ heading:'', body:'', bulletsHeading:'Key Benefits', bullets:[], imagePosition:'right', image:'' } },
  { type:'process',       emoji:'⚙️',  label:'Process Steps',       desc:'Our development process steps',          defaultData:{ title:'Our Development Process', steps:[] } },
  { type:'industries',    emoji:'🏭',  label:'Industries We Serve', desc:'Bullet list + photo grid',              defaultData:{ title:'Industries We Serve', bullets:[], images:[] } },
  { type:'text',          emoji:'✍️',  label:'Text Section',        desc:'Generic rich text with optional title',  defaultData:{ title:'', body:'' } },
  { type:'bullets',       emoji:'•',   label:'Bullet List',         desc:'Section title + list items',             defaultData:{ title:'', items:[] } },
  { type:'imageGrid',     emoji:'🌄',  label:'Image Grid',          desc:'Photo grid (2-4 columns)',               defaultData:{ images:[], columns:'3' } },
  { type:'cta',           emoji:'📣',  label:'CTA Banner',          desc:'Full-width call-to-action',              defaultData:{ headline:'', subheadline:'', btnLabel:'', btnUrl:'', bgImage:'' } },
  { type:'twoColumn',     emoji:'⬛⬛', label:'Two Columns',         desc:'Left + right content',                   defaultData:{ leftBody:'', rightType:'text', rightBody:'', rightImage:'' } },
  { type:'buttons',       emoji:'🔘',  label:'Buttons',             desc:'Row of CTA buttons',                     defaultData:{ buttons:[] } },
  { type:'divider',       emoji:'—',   label:'Divider / Space',     desc:'Spacing or horizontal rule',             defaultData:{ spacing:'md' } },
];

const TYPE_CLS: Record<ServiceBlockType, string> = {
  hero:e.typeHero, intro:e.typeIntro, cards:e.typeCards, textWithImage:e.typeTextWithImage,
  process:e.typeProcess, industries:e.typeIndustries, text:e.typeText, bullets:e.typeBullets,
  imageGrid:e.typeImageGrid, cta:e.typeCta, twoColumn:e.typeTwoColumn, divider:e.typeDivider, buttons:e.typeButtons,
};

function uid() { return Math.random().toString(36).slice(2,10); }

function BlockRouter({ type, data, onChange }: { type:ServiceBlockType; data:Record<string,unknown>; onChange:(d:Record<string,unknown>)=>void }) {
  switch(type) {
    ...
    case 'cards':         return <CardsBlockEditor         data={data} onChange={onChange}/>;
    case 'textWithImage': return <TextWithImageBlockEditor data={data} onChange={onChange}/>;
    case 'process':       return <ProcessBlockEditor       data={data} onChange={onChange}/>;
    case 'industries':    return <IndustriesBlockEditor    data={data} onChange={onChange}/>;
    ...
    default:              return <div style={{padding:12,fontSize:13,color:'var(--color-text-muted)'}}>Unknown block type</div>;
  }
}

function getLabel(type: ServiceBlockType, data: Record<string,unknown>): string {
  const t = data.title as string|undefined;
  const h = data.heading as string|undefined;
  const hl = data.headline as string|undefined;
  if (t?.trim()) return t.trim().slice(0,60);
  if (h?.trim()) return h.trim().slice(0,60);
  if (hl?.trim()) return hl.trim().slice(0,60);
  if (type==='intro') return 'Introduction paragraph';
  if (type==='divider') return `Spacer (${data.spacing??'md'})`;
  return BLOCK_TYPES.find(b=>b.type===type)?.label ?? type;
}

interface Block { id:string; type:ServiceBlockType; isVisible:boolean; collapsed:boolean; data:Record<string,unknown>; }
interface PageMeta { subServiceId:string|null; subServiceName:string; categoryId:string; categoryName:string; slug:string; isActive:boolean; isCategoryPage?:boolean; }

function ServicePageEditorInner({ pageId }: { pageId: string }) {
  ...
  useEffect(() => {
    let cancelled = false;
    (async () => {
      const res = await fetch(`/api/admin/services/pages/${pageId}`);
      const d = await res.json();
      if (cancelled) return;
      if (d.success) {
        const p = d.data;
        const isCatPage = p.pageType === 'category';
        setMeta({
          subServiceId: p.subServiceId?._id ?? p.subServiceId ?? null,
          subServiceName: isCatPage
            ? (p.categoryId?.name ?? 'Category Page')
            : (p.subServiceId?.name ?? 'Unknown'),
          categoryId: p.categoryId?._id ?? p.categoryId,
          categoryName: p.categoryId?.name ?? 'Unknown',
          slug: p.slug,
          isActive: p.isActive,
          isCategoryPage: isCatPage,
        });
        setIsActive(p.isActive);
        setBlocks((p.contentBlocks ?? []).map((b: { type:ServiceBlockType; isVisible:boolean; data:Record<string,unknown> })=>({ id:uid(), type:b.type, isVisible:b.isVisible??true, collapsed:false, data:b.data??{} })));
      } else { toast('error','Failed to load page'); router.push('/admin/services/sub-services'); }
      setLoading(false);
    })();
    return () => { cancelled = true; };
  }, [pageId]); // eslint-disable-line react-hooks/exhaustive-deps

  const addBlock = (type: ServiceBlockType) => {
    const meta2 = BLOCK_TYPES.find(b=>b.type===type);
    setBlocks(bs=>[...bs, { id:uid(), type, isVisible:true, collapsed:false, data:{...meta2?.defaultData??{}} }]);
    setShowAdd(false);
  ...
  const onDragEnd   = () => { if(dragIdx.current!==null&&dragOver.current!==null&&dragIdx.current!==dragOver.current) moveBlock(dragIdx.current,dragOver.current); dragIdx.current=null; dragOver.current=null; };
  ...
  const handleSave = async () => {
    ...
    const res = await fetch(`/api/admin/services/pages/${pageId}`, { method:'PATCH', headers:{'Content-Type':'application/json'}, body:JSON.stringify(payload) });
    const d = await res.json();
    if (d.success) {
      toast('success','Page saved!');
      router.push(meta?.isCategoryPage ? '/admin/services/categories' : '/admin/services/sub-services');
      return;
    }
    setSaving(false);
      ...
      <div className={e.editorMain}>
        <header className={e.editorHeader}>
          <div className={e.editorHeaderLeft}>
            <span className={e.editorHeaderTitle}>{meta?.subServiceName ?? 'Service Page'}</span>
            <span className={e.editorHeaderBreadcrumb}>Services / {meta?.isCategoryPage ? 'Categories' : meta?.categoryName} / {meta?.subServiceName}</span>
          </div>
          <div className={e.editorHeaderActions}>
            <button className={`${e.btn} ${e.btnSecondary}`} onClick={()=>router.push(meta?.isCategoryPage ? '/admin/services/categories' : '/admin/services/sub-services')} disabled={saving}><BackIcon/> Back</button>
        ...
        <div className={e.editorBody}>
          ...
          <div className={e.editorLeft}>
            ...
            <div className={e.listingCard}>
              <div className={e.listingCardTitle}>📋 Page Information</div>
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12,fontSize:13}}>
                <div><div style={{fontSize:11,fontWeight:700,color:'var(--color-text-muted)',marginBottom:3}}>{meta?.isCategoryPage ? 'CATEGORY PAGE' : 'SERVICE'}</div><div style={{fontWeight:600}}>{meta?.subServiceName}</div></div>
                <div><div style={{fontSize:11,fontWeight:700,color:'var(--color-text-muted)',marginBottom:3}}>CATEGORY</div><div style={{fontWeight:600}}>{meta?.categoryName}</div></div>
                <div><div style={{fontSize:11,fontWeight:700,color:'var(--color-text-muted)',marginBottom:3}}>SLUG</div><code style={{fontSize:12,background:'var(--color-bg)',padding:'2px 6px',borderRadius:4,color:'var(--color-primary)'}}>{meta?.slug}</code></div>
            ...
            <div>
              <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:8}}>
                <div style={{fontSize:13,fontWeight:700,color:'var(--color-text-secondary)'}}>
                  Content Blocks <span style={{fontSize:12,fontWeight:500,color:'var(--color-text-muted)'}}>({blocks.length})</span>
                </div>
              ...
              {blocks.length===0&&!showAdd&&(
                <div style={{padding:'32px 20px',textAlign:'center',background:'var(--color-surface)',border:'2px dashed var(--color-border)',borderRadius:'var(--radius-lg)',color:'var(--color-text-muted)',fontSize:13}}>
                  No content blocks.<br/><strong>Click &quot;Add Block&quot;</strong> to start building this service page.
                </div>
              )}

              <div className={e.blockList}>
                {blocks.map((block,idx)=>{
                  const bm=BLOCK_TYPES.find(bt=>bt.type===block.type);
                  return (
                    <div key={block.id}
                      className={`${e.blockCard}${!block.isVisible?' '+e.blockCardHidden:''}${block.collapsed?' '+e.blockCollapsed:''}`}
                      draggable onDragStart={()=>onDragStart(idx)} onDragEnter={()=>onDragEnter(idx)} onDragEnd={onDragEnd} onDragOver={ev=>ev.preventDefault()}>
                      <div className={e.blockCardHeader}>
                        <span className={e.blockDragHandle} title="Drag to reorder"><GripIcon/></span>
                        <span className={`${e.blockTypeBadge} ${TYPE_CLS[block.type]}`}>{bm?.emoji} {bm?.label}</span>
          ...
          <div className={e.editorRight}>
            <div className={e.sideCard}>
              ...
              <div className={e.sideCardBody} style={{gap:5,padding:'10px 14px'}}>
                {blocks.length===0
                  ...
                  :blocks.map((b,i)=>{
                    const bm=BLOCK_TYPES.find(bt=>bt.type===b.type);
                    return (
                      <div key={b.id} style={{display:'flex',alignItems:'center',gap:7,fontSize:12,cursor:'pointer',padding:'2px 0'}} onClick={()=>toggleCollapse(b.id)}>
                        ...
                        <span style={{flex:1,color:b.isVisible?'var(--color-text-secondary)':'var(--color-text-muted)',fontWeight:600,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{bm?.emoji} {bm?.label}</span>
            ...
            <div className={e.sideCard}>
              ...
              <div className={e.sideCardBody} style={{gap:5}}>
                {BLOCK_TYPES.map(bt=>(
                  <button key={bt.type} className={`${e.btn} ${e.btnSecondary} ${e.btnSm}`} type="button" style={{width:'100%',justifyContent:'flex-start',gap:7}} onClick={()=>addBlock(bt.type)}>
                    {bt.emoji} {bt.label}
                  </button>
                ))}
              </div>
...
export default function ServicePageEditor({ pageId }: { pageId: string }) {
  return <ToastProvider><ServicePageEditorInner pageId={pageId}/></ToastProvider>;
}
