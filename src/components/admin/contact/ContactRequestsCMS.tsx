...
function ContactRequestsCMSInner() {
    const { toast } = useToast();
    const [requests, setRequests] = useState<ContactReq[]>([]);
    const [loading, setLoading] = useState(true);
    const [search, setSearch] = useState('');
    const [statusF, setStatusF] = useState('all');
    const [deleteItem, setDeleteItem] = useState<ContactReq | null>(null);
    ...
    const fetch_ = async (q = search, st = statusF) => {
        setLoading(true);
        const p = new URLSearchParams({ ...(st !== 'all' ? { status: st } : {}), ...(q ? { search: q } : {}) });
        const res = await fetch(`/api/admin/contact?${p}`);
    ...
    const cycleStatus = async (req: ContactReq) => {
        const next = STATUS_NEXT[req.status];
        const res = await fetch(`/api/admin/contact/${req._id}`, {
            method: 'PATCH',
    ...
    const handleDelete = async () => {
        ...
        setDelLoading(true);
        const res = await fetch(`/api/admin/contact/${deleteItem._id}`, { method: 'DELETE' });
        const d = await res.json();
        ...
        <div className={s.cmsPage}>
            ...
            <div className={s.cmsMain}>
                <header className={s.cmsHeader}>
                    <div className={s.cmsHeaderLeft}>
                        ...
                        <span className={s.cmsHeaderBreadcrumb}>Enquiries / All Requests</span>
                    </div>
                </header>
                    ...
                    <div className={s.tableWrapper}>
                        {loading ? (
                            ...
                            <div className={s.emptyState}>
                                ...
                                <div className={s.emptyDesc}>Contact form submissions from the website will appear here.</div>
                            </div>
                        ) : (
                            <table className={s.table}>
                                <thead>
                                ...
                                <tbody>
                                    {requests.map(req => (
                                        <React.Fragment key={req._id}>
                                            <tr
                                                className={s.tr}
                                                style={{ cursor: 'pointer', background: req.status === 'new' ? 'var(--color-primary-pale, #f0fdf8)' : undefined }}
                                                onClick={() => setExpanded(expanded === req._id ? null : req._id)}
                                            >
                                                <td className={s.td} style={{ fontWeight: req.status === 'new' ? 700 : 500 }}>{req.name}</td>
                                                <td className={s.td} style={{ fontSize: 12 }}>
                                                    <a href={`mailto:${req.email}`} onClick={e => e.stopPropagation()} style={{ color: 'var(--color-primary)', textDecoration: 'none' }}>
                                                        {req.email}
                                                    </a>
                                                </td>
                                                <td className={s.td} style={{ fontSize: 12 }}>{req.phone}</td>
                                                <td className={s.td}>
                                                    {req.requestNda ? (
                                                        <span className={`${s.badge} ${s.badgeActive}`} style={{ fontSize: 10 }}>NDA</span>
                                                    ) : (
                                                ...
                                                <td className={s.td}>
                                                    <span
                                                        className={badgeClass(req.status)}
                                                        style={{ cursor: 'pointer' }}
                                                        onClick={e => { e.stopPropagation(); cycleStatus(req); }}
                                                        title="Click to change status"
                                                    >
                                                        {STATUS_LABEL[req.status]}
                                                    </span>
                                                </td>
                                                <td className={s.td} style={{ fontSize: 11, color: 'var(--color-text-muted)', whiteSpace: 'nowrap' }}>{fmtDate(req.createdAt)}</td>
                                                <td className={s.td} style={{ textAlign: 'right' }}>
                                                    <div className={s.actions}>
                                                        <button
                                                            className={`${s.btn} ${s.btnSecondary} ${s.btnSm}`}
                                                            onClick={e => { e.stopPropagation(); setExpanded(expanded === req._id ? null : req._id); }}
                                                            title="View details"
                                                        >
                                                            <EyeIcon /> View
                                                        </button>
                                                        <button
                                                            ...
                                                            onClick={e => { e.stopPropagation(); setDeleteItem(req); }}
                                                        >
                                                            <TrashIcon />
                                                        </button>
                                                    </div>
                                                ...
                                                <tr className={s.tr}>
                                                    <td colSpan={7} style={{ padding: '0 16px 16px' }}>
                                                        <div style={{
                                                            ...
                                                            <div style={{ marginTop: 12, display: 'flex', gap: 16, fontSize: 12, color: 'var(--color-text-muted)' }}>
                                                                <span>✅ Agreed to Privacy Policy</span>
                                                                {req.requestNda && <span>🔒 Requested NDA</span>}
                                                            </div>
                                                        </div>
                                                    </td>
                                                </tr>
            ...
            {deleteItem && (
                <ConfirmDeleteModal
                    ...
                    message={`Request from "${deleteItem.name}" will be permanently deleted.`}
...
export default function ContactRequestsCMS() {
    return <ToastProvider><ContactRequestsCMSInner /></ToastProvider>;
}